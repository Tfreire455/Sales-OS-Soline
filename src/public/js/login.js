...
document.addEventListener("DOMContentLoaded", () => {
	...
	function createParticles() {
		...
		const count = prefersReducedMotion ? 0 : isMobile ? 10 : 26;
		...
		for (let i = 0; i < count; i++) {
			...
			const size = isMobile ? Math.random() * 2 + 1 : Math.random() * 4 + 1;
			const duration =
				Math.random() * (isMobile ? 2 : 3) + (isMobile ? 2 : 2.5);
			const delay = Math.random() * (isMobile ? 4 : 5);
		...
		if (card) {
			...
			document.addEventListener("mouseleave", () => {
				...
				card.addEventListener(
					...
					{ once: true },
				); // ← { once: true } remove o listener automaticamente
			});
		}
	}
});